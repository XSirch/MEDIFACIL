...
def abrir_horario(request):
    ...
    if request.method == "GET":
        ...
        data_formatada = datetime.datetime.strptime(data, "%Y-%m-%dT%H:%M")
        
        if data_formatada <= datetime.datetime.now():
...
def consultas_medico(request):
    ...
    consultas_hoje = Consulta.objects.filter(data_aberta__user=request.user).filter(data_aberta__data__gte=hoje).filter(data_aberta__data__lt=hoje + datetime.timedelta(days=1))
...
def dashboard(request):
    ...
    .filter(data_aberta__data__range=[datetime.datetime.now().date() - datetime.timedelta(days=7), datetime.datetime.now().date() + datetime.timedelta(days=1)])\
...
def planos(request):
    if request.method == "GET":
        return render(request, 'planos.html',{'is_medico': is_medico(request.user)})
